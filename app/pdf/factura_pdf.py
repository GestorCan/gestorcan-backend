...
def campo(pdf, etiqueta, valor, ancho, salto=False):

    etiqueta = limpiar_texto_pdf(etiqueta)
    ...
    pdf.cell(ancho - 28, 6, str(valor or ""), 0, 1 if salto else 0)

def campo2(pdf, etiqueta, valor, ancho, salto=False):
    ...
    pdf.set_font("Arial", "", 10)
    pdf.cell(ancho - 32, 6, str(valor or ""), 0, 1 if salto else 0)
...
def generar_pdf_factura(factura, estancia, cliente, mascota, output_path=None):
    ...
    numero_visible = factura.numero_factura

    if "-" in str(numero_visible):
        numero_visible = str(numero_visible).split("-")[-1]

    pdf.cabecera("Factura", numero_visible, fecha=factura.fecha)
    # ===== CLIENTE =====
    ...
    # ===== SERVICIOS =====
    #pdf.bloque_titulo("Servicios y Totales (Iva Incluido)")
    ...
    pdf.cell(0, 5, "(IVA incluido)", ln=True)
    ...
    campo(pdf, "Extras", estancia.extras or "Sin extras", 65)
    campo(pdf, "Importe Extras", f"{float(estancia.importe_extras or 0):.2f} EUR", 60, salto=True)
    ...
    pdf.cell(0,8,f"Base Imponible: {float(factura.base_imponible):.2f} EUR",0,1,"R" )

    pdf.cell(0,8,f"IVA 21%: {float(factura.iva):.2f} EUR",0,1,"R")

    pdf.cell(0,10,f"TOTAL FACTURA: {float(factura.total):.2f} EUR",0,1,"R")
    ...
    if factura.qr_path and os.path.exists(factura.qr_path):
        ...
        pdf.cell(
            0,
            5,
            "Factura generada conforme al sistema VeriFactu",
            0,
            1,
            "L"
        )
    ...
    if output_path is None:
        ...
        anio = factura.fecha.year if factura.fecha else "sin_fecha"

        carpeta_facturas = BASE_DIR / "media" / "facturas" / str(anio)
        carpeta_facturas.mkdir(parents=True, exist_ok=True)

        output_path = carpeta_facturas / f"factura_{factura.numero_factura}.pdf"
    else:
